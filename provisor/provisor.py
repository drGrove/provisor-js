...
class Provisor(object):
    ...
    def list_users(self):
        ...
        results = self.con.search_s(
            self.user_base, ldap.SCOPE_ONELEVEL, '(objectClass=*)', ("uid",), 0)
        for r in results:
            for attrs in r[1]:
    ...
    def server_stats(self):
        ...
        for server in server_list:
            ...
            stats[server['cn']]['currentUsers'] = stats[server['cn']].get(
                'currentUsers', 0)
        ...
        for r in user_results:
            for attrs in r[1]:
                host = r[1][attrs][0]
                if host in stats:
                    stats[host]['currentUsers'] = stats[host].get(
                        'currentUsers', 0) + 1
    ...
    def list_groups(self):
        groups = []
        results = self.con.search_s(
            self.group_base, ldap.SCOPE_ONELEVEL, '(objectClass=*)', ("cn",), 0)
        for r in results:
            for attrs in r[1]:
    ...
    def next_uid(self):
        uids = []
        results = self.con.search_s(
            self.user_base, ldap.SCOPE_ONELEVEL, '(objectClass=*)', ("uidNumber",), 0)
        for r in results:
            for attrs in r[1]:
    ...
    def modify_user(self, username, pubkeys=None,
        ...
        if password:
            password = '{crypt}' + \
                crypt.crypt(password, "$6${0}".format(make_salt()))
