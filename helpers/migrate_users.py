...
valid_pkey = re.compile(
    '(ssh\-(rsa|dsa|ecdsa) [a-zA-Z0-9+/=]{180,5000})( .*$)?')
# Valid shells
valid_shells = []
...
def main():
    ...
    for u in allusers:
        ...
        try:
            ...
            sys.stderr.write(
                "Cannot import {0} as cannot find entry in group or shadow database\n".format(u.pw_name))
            continue
        ...
        try:
            ...
            if not pubkey:
                sys.stderr.write(
                    "Unable to migrate '{0}' as the user has no valid public key\n".format(u.pw_name))
                continue
        except:
            sys.stderr.write(
                "Unable to migrate '{0}' as the pubkey file was not accessible\n".format(u.pw_name))
            continue

        # Time to validate some entries
        if not valid_user.match(u.pw_name):
            sys.stderr.write(
                "The username '{0}' did not validate as the username is invalid\n".format(u.pw_name))
            continue
        ...
        if not valid_pkey.match(pubkey):
            sys.stderr.write(
                "The username '{0}' did not validate as the public key is invalid\n".format(u.pw_name))
            continue
        else:
        ...
        if u.pw_shell not in valid_shells:
            sys.stderr.write("The username '{0}' did not validate as the shell '{1}' is invalid\n".format(
                u.pw_name, u.pw_shell))
            continue
        # Validate shadow details..
        if s.sp_lstchg < 1000 or s.sp_lstchg > nowday:
            sys.stderr.write(
                "The username '{0}' did not validate as the users shadow last change field is invalid\n".format(u.pw_name))
            continue
